...
from src.eval.utils import concatenate_scores, saveResult_acrossTasks
...
def inference(
    ...
    if scores is not None:
        ...
        saveTo_gcp(
            evaluation_config.should_save_evaluation_run_to_gcp,
            inferenceScores_fp + ".json",
        )
...
if __name__ == "__main__":
    ...
    if args.output_dir is None:
        # Means we are doing inference on pretrained model
        if model_config.filepath_to_load_model is None:
            experiment_dir = "exp_out"
            experiment_dir = os.path.join(
                experiment_dir,
                evaluation_config.get_datasetConfig().get_experimentDir(),
            )
            experiment_dir = os.path.join(
                experiment_dir, model_config.get_experimentDir()
            )

        else:
            # Experiment directory for inference is created for checkpoint idx
            if args.merged_model is None:
                experiment_dir = os.path.dirname(
                    os.path.dirname(model_config.filepath_to_load_model)
                )
                checkpoint_name = os.path.basename(model_config.filepath_to_load_model)
                experiment_dir = os.path.join(experiment_dir, checkpoint_name)
            # Experiment directory if we just load merged model
            else:
                experiment_dir = os.path.dirname(model_config.filepath_to_load_model)
    else:
