...
def greedyGeneration_decoder(
    ...
    """
    Assumes model is decoder model and caches input first
    ...
    inputIds_ignoreLastOne = input_ids[:, :-1]
    inputMask_ignoreLastOne = input_mask[:, :-1]

    # Last input_ids use as first id to start generating from decoder
    last_inputid = input_ids[:, -1:]
    last_inputMask = input_mask[:, -1:]

    output = transformer(
        input_ids=inputIds_ignoreLastOne,
        attention_mask=inputMask_ignoreLastOne,
        use_cache=True,
    )
    ...
    current_decoderInputIds = last_inputid
    # Input mask consist of all the cached key_values and current input_ids
    current_decoderMask = input_mask
    ...
    for i in range(max_generationLength):
        output = transformer(
            input_ids=current_decoderInputIds,
            attention_mask=current_decoderMask,
            use_cache=True,
            past_key_values=past_key_values,
        )
