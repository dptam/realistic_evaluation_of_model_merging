...
from src.eval.evaluation import evaluate_fromConfig, evaluate_onDatasets
...
def train(device, training_config: TrainingConfig):
    ...
    for i in tqdm(range(training_config.num_batches * gradient_accumulation_factor)):
        ...
        if is_distributedSetup(world_size):
            ...
            if is_nodeZero(device):
                batch_metrics = reduce_gatheredOutput(
                    gathered_currentMetrics, get_average
                )

        if is_nodeZero(device):
            effectiveBatch_metrics = addValues_inDict(
                effectiveBatch_metrics, batch_metrics
            )
            ...
            if (batch_idx + 1) % training_config.checkpoint_frequency == 0:
                ...
                if training_config.use_early_stopping:
                    if (
                        ...
                        saveExperiment_toGCP(training_config)
                        return
