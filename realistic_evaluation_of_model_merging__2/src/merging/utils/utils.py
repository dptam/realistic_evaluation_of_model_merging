    ...
    if merging_config.mixture_subset_max_id is not None:
        ...
        for mixture_subset_id in list(
            ...
            if new_experiment_name is None:
                ...
                new_experiment_name = new_experiment_name + f"/mixture_subset_{mixture_subset_id}"
        ...
        if merging_config.mixture_subset_id is not None:
            new_experiment_name = merging_config.experiment_name
            if new_experiment_name is None:
                new_experiment_name = f"mixture_subset_{merging_config.mixture_subset_id}"
            else:
                new_experiment_name = new_experiment_name + f"/mixture_subset_{merging_config.mixture_subset_id}"
            all_mergingConfig.append(
                update_mergingConfig(
                    merging_config,
                    {
...
def getFinal_parameterNames(
    model_config: ModelConfig, merging_config: MergingConfig, parameter_names: list[str]
) -> Dict[str, Any]:

    final_parameterNames = {}

    for parameter_name in parameter_names:
        # Compute the merged weights for parameter
        if merging_config.merge_peft_weights:
            if model_config.merge_lora:
                assert "lora" in parameter_name

                # For LoRA, when loading A matrix, additionally load the B matrix and merge. Then when loading B matrix, throw error
                if "A" in parameter_name:
                    ...
                    final_parameterNames[final_parameterName] = [
                        parameter_name,
                        B_parameterName,
                    ]
                else:
            ...
            final_parameterNames[parameter_name] = [parameter_name]

    return final_parameterNames
