...
def save_trimmedModel(
    ...
    if checkCheckpointSaveName_exists(checkpointStatistic_name):
        return
...
def save_fisher(
    ...
    if merging_config.fisher_approximation == "input_activations_covariance":
        ...
        if model_config.peft_method == "lora":
            ...
            module_toSearchFor = model_config.target_modules
        else:
            if model_config.peft_method is not None:
                raise ValueError(
                    f"Invalid peft method {model_config.peft_method} for input activations covariance"
                )

        for module_name, module in model.named_modules():
            # For PEFT methods, we save the input activations to the PEFT module and for non-PEFT methods, we save the input activations to the linear layer
            if (
                model_config.peft_method is not None
                and re.search(module_toSearchFor, module_name)
                and isinstance(module, nn.Linear)
            ) or (
                model_config.peft_method is None
                and (
            ...
            with torch.no_grad():
                ...
                for module_name, activations in stored_inputActivations.items():
                    ...
                    if model_architecture == "decoder":
                        number_ofTokens = torch.sum(batch["decoder_input_mask"])
